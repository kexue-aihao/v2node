...
# GitHub 仓库（fork 可改此处或通过环境变量覆盖）
V2NODE_REPO="${V2NODE_REPO:-kexue-aihao/v2node}"
V2NODE_BRANCH="${V2NODE_BRANCH:-master}"
V2NODE_RAW_BASE="https://raw.githubusercontent.com/${V2NODE_REPO}/${V2NODE_BRANCH}"
V2NODE_GITHUB_BASE="https://github.com/${V2NODE_REPO}"
...
install_v2node() {
    ...
    if  [[ -z "$version_param" ]] ; then
        last_version=$(curl -Ls "https://api.github.com/repos/${V2NODE_REPO}/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        url="${V2NODE_GITHUB_BASE}/releases/download/${last_version}/v2node-linux-${arch}.zip"
...
EOF
    ...
    curl -o /usr/bin/v2node -Ls "${V2NODE_RAW_BASE}/script/v2node.sh"
