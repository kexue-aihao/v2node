...
install() {
    bash <(curl -Ls "${V2NODE_RAW_BASE}/script/install.sh")
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls "${V2NODE_RAW_BASE}/script/install.sh") $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/v2node -N --no-check-certificate "${V2NODE_RAW_BASE}/script/v2node.sh"
    if [[ $? != 0 ]]; then
        echo ""
